...
const Footer = () => {
  ...
  const linkSections = [
    {
      title: "SHOP",
      links: [
        { text: "For Her", path: "/gender/female" },
    ...
    },
    {
      title: "CONTACT",
      links: [
        { text: "+234-800-000-0000", path: "/", icon: PhoneIcon },
        { text: "hello@laysscent.com", path: "/", icon: MailIcon },
  ...
  return (
    <footer className="bg-[#0a0706] border-t border-white/8">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        {/* Main footer content */}
        <div className="flex flex-col md:flex-row items-start justify-between gap-12 py-14 border-b border-white/8">
          {/* Brand column */}
          <div className="max-w-xs">
            ...
            <p className="text-sm text-white/35 leading-relaxed mb-8">
              Premium fragrances for every story. From floral to oud, we bring
              you the finest scents from around the world — curated for her,
              him, and everyone in between.
            </p>

            {/* Newsletter */}
            <p className="text-[11px] tracking-[2px] uppercase text-[#c97b63] mb-3">
              Stay in the loop
            </p>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-white/5 border border-white/10 rounded-full px-4 py-2 text-sm text-white placeholder-white/25 outline-none focus:border-[#c97b63]/50 transition-colors"
              />
              <button className="bg-[#c97b63] hover:bg-[#b56d55] text-white rounded-full px-4 py-2 text-sm transition-colors">
                →
              </button>
            </div>
            ...
            <div className="flex items-center gap-2 mt-6">
              {socialIcons.map((item, i) => (
                <Link
                  href={item.link}
                  key={i}
                  className="flex items-center justify-center w-9 h-9 rounded-full border border-white/10 text-white/40 hover:border-[#c97b63]/50 hover:text-[#c97b63] transition-all duration-300"
                >
                  <item.icon />
                </Link>
              ))}
          ...
          <div className="flex flex-wrap justify-between w-full md:w-[50%] gap-8 text-sm">
            {linkSections.map((section, index) => (
              <div key={index}>
                <h3 className="text-[10px] tracking-[3px] uppercase text-[#c97b63] mb-5">
                  {section.title}
                </h3>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 py-5">
          <p className="text-xs text-white/20">
            © 2025 Lays Scent. All rights reserved.
          </p>
          <div className="flex items-center gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-[#c97b63]/50" />
            <p className="text-xs text-white/20">
              Premium fragrances • Lagos, Nigeria
            </p>
          </div>
        </div>
      </div>
